...
- **Keyboard**: `⌘/Ctrl+Z` undo, `⇧⌘/Ctrl+Z` or `⌘/Ctrl+Y` redo, `⌫/Delete` remove selection, `⌘/Ctrl+C` / `⌘/Ctrl+V` copy/paste nodes/edges/markups, `Esc` clears selection or exits tools, `←/↑/→/↓` nudge selected nodes by 10 px (hold `Shift` for 1 px), and `←/→` adjust a selected edge’s midpoint horizontally with the same coarse/fine steps.
