...
export const Button = React.forwardRef<HTMLButtonElement, Props>(function Button({ className='', variant='default', size='md', ...props }, ref) {
  ...
  const variants: Record<string,string> = { default:'bg-sky-600 text-white hover:bg-sky-700', outline:'border border-slate-300 bg-white hover:bg-slate-50', ghost:'hover:bg-slate-100' }
  const sizes: Record<string,string> = { sm:'text-sm px-3 py-1.5', md:'', lg:'text-lg px-5 py-3' }
