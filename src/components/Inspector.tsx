...
import { CanvasMarkup, DualOutputConverterBranch, DualOutputConverterNode, Edge, Project } from '../models'
...
export default function Inspector({selection, onDeleted, onOpenSubsystemEditor, onSelect}:{selection:InspectorSelection|null, onDeleted?:()=>void, onOpenSubsystemEditor?:(id:string)=>void, onSelect?:(selection:InspectorSelection)=>void}){
  ...
  const updateEdgeGroup = useStore(s=>s.updateEdges as any)
  ...
  if (edge) {
    const groupKeyValue = edgeGroupKey({ from: edge.from, fromHandle: edge.fromHandle })
    const groupedEdges = project.edges.filter(e => edgeGroupKey({ from: e.from, fromHandle: e.fromHandle }) === groupKeyValue)
    const edgeGroupIds = groupedEdges.map(e => e.id)
    const applyEdgeGroupPatch = (patch: Partial<Edge>) => {
      if (!edgeGroupIds.length) return
      if (updateEdgeGroup) {
        updateEdgeGroup(edgeGroupIds, patch)
      } else if (updateEdge) {
        edgeGroupIds.forEach(id => updateEdge(id, patch))
      }
    }
    ...
    return (
      <InspectorShell>
        ...
        <InspectorContent>
          ...
          <InspectorSection title="Appearance" description="Overrides modify the rendered edge atop the voltage-based default.">
            <FormGrid columns={2}>
